...
bool testEllipse2d()
{
	...
	ellipse2D.setMaxIterations(25000);
	ellipse2D.setSpecficAxisLength(135, 85, 0.2);
	...
	cv::ellipse(showMat, cv::RotatedRect(ellipseCenter, ellipseSize, ellipseAngle), cv::Scalar(0, 0, 255), 1, 8);
...
bool testEllipse2dMulti()
{
	...
	while (pCloud2D.size() > 500)
	{
		...
		//cout << "Long/Short Axis: " << parameter.modelParam[2] << "/" << parameter.modelParam[3] << " --- ";
		//cout << "Angle: " << parameter.modelParam[4] << endl;
		//cv::ellipse(showMat, cv::RotatedRect(ellipseCenter, ellipseSize, ellipseAngle), cv::Scalar(0, 0, 255), 3, 8);
